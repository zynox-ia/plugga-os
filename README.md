...
- **Migrações e seed** não rodam sozinhas. Aplique-os deliberadamente dentro
  do container: `docker compose exec api pnpm db:migrate:deploy` e
  `docker compose exec api pnpm db:seed`. Para o primeiro bootstrap, defina
  `SEED_ADMIN_EMAIL` e uma senha forte em `.env`; após o seed, retire
  `SEED_ADMIN_PASSWORD` do arquivo e recrie somente a API
  (`docker compose up -d --force-recreate api`). Assim um re-seed futuro não
  troca a senha administrativa silenciosamente.
